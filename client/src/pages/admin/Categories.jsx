        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">Manage item classifications and their icons</p>
                </div>
                <button
            ...
            <div className="bg-white/50 backdrop-blur-sm border border-primary-100 p-6 rounded-[2rem] flex flex-col sm:flex-row gap-4 text-primary-900 mb-8 shadow-sm">
                ...
                <div>
                    <p className="text-sm font-black uppercase tracking-widest text-primary-600 mb-1">Categorization Logic</p>
                    <p className="text-xs font-medium leading-relaxed text-primary-800/80">
                        Categories help users filter inventory and reports faster. Removing a category will not delete the items in it, but they will be marked as <span className="font-bold underline">"Uncategorized"</span>.
                    </p>
                </div>
            </div>
            ...
            {loading ? (
                ...
                <div className="bg-white rounded-[3rem] p-16 border border-gray-100 shadow-sm flex flex-col items-center text-center">
                    <EmptyState
                        icon="📂"
                        title="No categories"
                        description="Create your first category to help users organize their items."
                    />
                </div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    {categories.map((cat) => (
                        <div key={cat.id} className="bg-white rounded-[2.5rem] p-6 border border-gray-100 group relative shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
                            ...
                            <div className="mt-6">
                                <h3 className="text-lg font-black text-gray-900 group-hover:text-primary-600 transition-colors tracking-tight">{cat.name}</h3>
                                <div className="flex items-center gap-2 mt-2">
                                    <div className="px-2.5 py-1 rounded-lg bg-gray-50 text-[10px] font-black text-gray-400 uppercase tracking-widest border border-gray-100">
                                        System Class
                                    </div>
            ...
            {showDeleteConfirm && (
                <ConfirmDialog
                    ...
                    message={`Are you sure you want to delete the "${categoryToDelete?.name}" category? This cannot be undone.`}
